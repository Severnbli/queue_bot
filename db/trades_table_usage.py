...
async def reg_trade_(sender_id: int, receiver_id: int, queue_info_id: int):
    ...
    await NotifyManager.add_message(
        Message(
            user_id=receiver_id,
            text=f'Пользователь с ником {sender_nick} предлагает тебе поменяться местами в очереди {queue_info}: '
                 f'ты на {sender_place} место, а он на {receiver_place}. Для подтверждения отправь /accept{trade_id}',
            is_check_news=False
        )
    )
